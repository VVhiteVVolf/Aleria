...
`item-register-providers.js` liest die Anbieterwappen separat aus `goodsTable.headerIcon` und `tradeCatalog.headerIcon`, ersatzweise aus dem Modulsymbol oder ausdrücklich hinterlegten Hierarchie-, Haus- und Zunftwappen. Szenen- und Gebäudebilder werden nicht als Wappen verwendet. Dasselbe Wappen erscheint auf der Anbieter-Kachel, im Reiter und am Sortimentskopf; ohne hinterlegtes Wappen bleibt der Anfangsbuchstabe. Diese Anzeigedaten beeinflussen weder Waren noch Kaufrevisionen und werden mit den Moduländerungen aktualisiert.
