...
function sanitizeBountyFileData(data = {}) {
  ...
  return {
    ...
    archiveSubtitle: String(data.archiveSubtitle || 'Herausgegeben im Namen des Königreichs Cenyr').trim(),
    ...
    threatLabel: String(data.threatLabel || 'Gefährlichkeitsstufe').trim(),
    threatLevel: clampBountyNumber(data.threatLevel, 4, 1, 5),
    threatText: String(data.threatText || 'Äußerst gefährlich').trim(),
    ...
    chargesTitle: String(data.chargesTitle || 'Tatvorwürfe').trim(),
    charges: sanitizeBountyCharges(data.charges),
    traitsTitle: String(data.traitsTitle || 'Eigenschaften').trim(),
    traits: sanitizeBountyCharges(data.traits),
    ...
    connectionsTitle: String(data.connectionsTitle || 'Verbindungen & Zugehörigkeiten').trim(),
    ...
    alliesTitle: String(data.alliesTitle || 'Verbündete').trim(),
    allies: imageList(data.allies, 'Verbündeter'),
    enemiesTitle: String(data.enemiesTitle || 'Feinde').trim(),
    enemies: imageList(data.enemies, 'Feind'),
    supportersTitle: String(data.supportersTitle || 'Mögliche Unterstützer').trim(),
    supporters: imageList(data.supporters, 'Unterstützer'),
    dangerTitle: String(data.dangerTitle || 'Gefährlichkeitsprofil').trim(),
...
function sanitizeCasteData(data = {}) {
  return {
    archiveLabel: String(data.archiveLabel || 'Kastenregister').trim(),
    documentCode: String(data.documentCode || '').trim(),
    categoryLabel: String(data.categoryLabel || 'Gesellschaftliche Kaste').trim(),
    ...
    infoTitle: String(data.infoTitle || 'Identität & Status').trim(),
    ...
    rolesTitle: String(data.rolesTitle || 'Gesellschaftliche Funktion & Rollen').trim(),
    roles: sanitizeCasteCards(data.roles, 8),
    skillsTitle: String(data.skillsTitle || 'Ausbildung, Fähigkeiten & Wissen').trim(),
    ...
    restrictionsTitle: String(data.restrictionsTitle || 'Pflichten, Verbote & Sanktionen').trim(),
    restrictions: sanitizeCasteTextRows(data.restrictions, 12),
    organizationTitle: String(data.organizationTitle || 'Hierarchie & Zugehörigkeit').trim(),
    organizationRows: sanitizeCasteOrganizationRows(data.organizationRows),
    representativesTitle: String(data.representativesTitle || 'Bekannte Angehörige').trim(),
