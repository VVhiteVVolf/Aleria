...
function reviseEntry(entry) {
  ...
  return {
    ...
    effect: 'Hauptangriff: 1W6 plus die üblichen Boni. Ein Folgeangriff: 1W4 ohne erneuten Attribut-, Waffen- oder Berserker-Schadensbonus. Jeder Hieb hat einen eigenen Trefferwurf.',
...
export function reconcileClassDamageRevisions(profile = {}) {
  ...
  if (!hasCondition && !collections.some(entries => entries?.some(entry => FORMULA_REVISIONS[entry?.id] || entry?.id === 'fenrir-twin-axe-flurry'))) return profile;
  return { ...profile,
