...
const LONG_REST_RECOVERY = new Set(['short-rest', 'long-rest', 'day', 'scene']);
...
export function recoverSceneRestResources(resources = [], restType = 'short', recoveryDayKey = '') {
  ...
  return (Array.isArray(resources) ? resources : []).map((resource, index) => {
    ...
    if (normalizedType === 'long' && next.recovery === 'day' && dayKey) next.recoveryDayKey = dayKey;
    return next;
  });
}

export function recoverSceneRestAbilities(abilities = [], restType = 'short') {
  const normalizedType = getSceneRestType(restType).id;
...
export function buildSceneRestParticipant(profile = {}, restType = 'short', options = {}) {
  ...
  const afterResources = recoverSceneRestResources(beforeResources, restType, options.recoveryDayKey);
  const beforeAbilities = (Array.isArray(profile.abilities) ? profile.abilities : []).map(cloneAbility);
  const afterAbilities = recoverSceneRestAbilities(beforeAbilities, restType);
