...
function renderFazitLineHeader(line, index, lineCount) {
  ...
  return `<div class="fazit-line-editor-head">
    ...
    <div class="fazit-line-order-actions" aria-label="Baustein bearbeiten">
      ...
      <button type="button" data-action="duplicate-fazit-line" data-line-id="${safeLineId}" title="Baustein duplizieren" aria-label="Baustein duplizieren">⧉</button>
...
function renderFazitLineShell(line, index, lineCount, bodyMarkup, footerMarkup = '', extraMarkup = '') {
  return `<section class="fazit-line-editor fazit-line-editor-${escapeHtml(line.kind)}" data-line-id="${escapeHtml(line.id)}">
  ...
  return `<article class="fazit-token-editor" data-token-id="${safeTokenId}" data-token-kind="${token.kind}">
    <div class="fazit-token-editor-head">
      ...
      <span class="fazit-token-editor-actions">
        ...
        <button type="button" data-action="duplicate-fazit-token" data-line-id="${safeLineId}" data-token-id="${safeTokenId}" title="Symbol duplizieren" aria-label="Symbol duplizieren">⧉</button>
...
function renderFazitSymbolPresetButtons(line) {
  return FAZIT_SYMBOL_PRESETS.map(preset => `<button type="button" class="fazit-symbol-preset" data-action="add-fazit-symbol-preset" data-preset="${preset.key}" data-line-id="${escapeHtml(line.id)}" title="${escapeHtml(preset.label)}">
...
function renderFazitTokenLineEditor(line, index, lineCount, personPickerMarkup) {
  ...
  const footer = `<div class="fazit-line-add-actions">
      ...
      <button type="button" class="module-editor-mini-btn" data-action="add-fazit-token" data-token-kind="symbol" data-line-id="${escapeHtml(line.id)}">＋ Eigenes Symbol</button>
    </div>
...
function renderFazitListLineEditor(line, index, lineCount) {
  ...
  const footer = `<button type="button" class="module-editor-mini-btn" data-action="add-fazit-list-item" data-line-id="${escapeHtml(line.id)}">＋ Listenpunkt</button>
