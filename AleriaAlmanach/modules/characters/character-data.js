...
function mergeCharacterRecords(primary, fallback) {
  ...
  const merged = {
    ...
    status: getCharacterStatusValue(first.status || second.status || ''),
    relevance: getCharacterRelevanceValue(first.relevance || second.relevance || ''),
    taxonomyPath: first.taxonomyPath || second.taxonomyPath || '',
    currentLocation: first.currentLocation || second.currentLocation || '',
    origin: first.origin || second.origin || '',
    plotNode: first.plotNode || second.plotNode || '',
...
function buildCharacterSearchText(char) {
  const parts = [
    ...
    getCharacterStatusLabel(char.status),
    getCharacterRelevanceLabel(char.relevance),
    char.role,
    char.taxonomyPath,
