...
function spellCard(entry) {
  const spell = createCatalogSpell(entry.id);
  ...
  return `<details class="spell-card" id="${entry.id}" data-spell data-section="${entry.section}" data-grade="${entry.level}" data-role="${escape(entry.role)}" data-search="${escape(searchText)}">
    ...
    <div class="spell-card-body">
      ...
      <footer class="spell-card-footer"><a href="../../AleriaAlmanach/AleriaAlmanach.html?zauberkatalog=${entry.id}">Im Charakterbogen-Archiv öffnen ↗</a><button type="button" data-action="copy-spell-link" data-spell-id="${entry.id}" aria-label="Link zu ${escape(entry.name)} kopieren">Link kopieren</button><small>Fassung ${entry.revision} · ${entry.sourceId}</small></footer>
    </div>
  </details>`;
}

export function renderSpellListPage(entries) {
  return `<!doctype html>
<html lang="de">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="#293b38"><meta name="description" content="72 Elementarismus-Zauber für Aleria: Wirkungen, Aktionskosten, Mana, Schaden und höhere Grade. Verbunden mit dem Charakterbogen-Archiv."><title>Elementarismus · Zauberverzeichnis · Aleria</title><link rel="icon" href="../assets/druidic/elementarismus.png"><link rel="stylesheet" href="../modules/book-shell/magic-shell.css"><link rel="stylesheet" href="../../Fonts/Arkanes-Alphabet/arcane.css"><link rel="stylesheet" href="../modules/spell-list/spell-list.css"><script type="module" src="../modules/spell-list/spell-list-controller.mjs"></script></head>
<body class="magic-page spell-list-page"><a class="magic-skip" href="#zauber">Zum Zauberverzeichnis</a>
<header class="magic-masthead"><a class="magic-brand" href="../../AleriaAlmanach/AleriaAlmanach.html">← ALERIA <i>/</i> ALMANACH</a><nav aria-label="Weitere Sammlungen"><a href="../index.html#elementarismus">Magie</a><a href="../../Klassenordner/Klassenseite.html">Klassen</a><a href="../../Religionen/index.html">Religionen</a></nav></header>
<main class="magic-book" data-spell-list>
  <header class="spell-cover"><div><a class="spell-breadcrumb" href="../index.html#druiden">Magie / Die druidischen Wege</a><p class="magic-eyebrow">Alerias Zauberverzeichnisse · Band I</p><h1>Elementarismus</h1><p class="spell-cover-subtitle">Die Welt in Bewegung.</p><p class="spell-cover-intro">Feuer wecken. Wasser lenken. Den Wind tragen lassen.<br>Ein Verzeichnis der Formen, mit denen die Elemente antworten.</p><div class="spell-cover-counts"><span><b>72</b> Zauber</span><span><b>6</b> Elemente</span><span><b>0–9</b> Grade</span></div></div><div class="spell-cover-sigil" aria-hidden="true"><span class="arcane">&#xE000; &#xE00C; &#xE01F;</span><img src="../assets/druidic/elementarismus.png" width="220" height="220" alt=""><small>IGNIS · AQUA · AER · TERRA</small></div></header>
  <details class="spell-conventions"><summary>Vor dem ersten Zauber <span>Mana · Aktionen · Konzentration</span></summary><div><p><b>Ein gemeinsames Regelwerk.</b> Mana folgt der bestehenden Gradstaffel: ${Array.from({length:10},(_,level)=>`${level}: ${getSpellManaCost(level)}`).join(' · ')}. Ein freigeschalteter Grad wird beim Wirken nicht verbraucht. Besondere Aktionen sind eine dauerhafte Ressource und erneuern sich nicht mit jedem Beitrag.</p><div class="spell-cost-legend">${actionCosts(['action','bonus-action','reaction','special-action'])}</div><p><b>Wirkung und Grenzen.</b> Ein höherer Grad verwendet nur seine aufgeführten Werte. Es gibt keine freie Steigerung einzelner Parameter und keine zusätzlichen Nässe-Schadenswürfel. Flächen können Verbündete treffen; Sicht, Wirkungslinie, vorhandenes Material und Größe gelten weiterhin.</p><p><b>Zeit am Spieltisch.</b> „Eigene Beiträge“ meint die Beiträge des Zaubernden. Konzentrationsabbruch wird im Kampf berücksichtigt; die angegebene Höchstdauer, Gelände und spätere Kontakte werden gemeinsam nachgehalten. Die hier aufgeführten Konzentrationszauber haben keine zusätzlichen Mana-Erhaltungskosten. Rituale werden erst nach der angegebenen Zeit als Abschluss verbucht; bei angekündigten Kampfzaubern ist der erste Beitrag Vorbereitung und der zweite die bezahlte Entladung.</p><p><b>Gemeinsam mit dem Charakterbogen.</b> Jeder Eintrag ist im Archiv unter Elementarismus verfügbar. Gelernte Katalogzauber behalten ihre Fassung; bereits vorhandene Zauber behalten ihre bisherigen Werte. Eigene Bearbeitungen werden als eigene Fassung geführt. Die Hinweise „Am Spieltisch“ benennen Wirkungen, die gemeinsam aufgelöst werden.</p></div></details>
  <div class="spell-register-layout" id="zauber"><aside class="spell-register"><p class="magic-eyebrow">Das Verzeichnis</p><nav aria-label="Elementare Ausrichtungen">${SPELL_CATALOG_SECTIONS.map((section,index)=>`<a href="#element-${section.id}"><span>${String(index+1).padStart(2,'0')}</span>${escape(section.name)}<small>${entries.filter(entry=>entry.section===section.id).length}</small></a>`).join('')}</nav><p>Die Elemente werden einzeln erlernt. Donner und Blitz sind eigene Ausrichtungen.</p><a href="../../AleriaAlmanach/AleriaAlmanach.html?zauberkatalog=elementarismus">Zum Charakterbogen-Archiv ↗</a></aside>
  <div class="spell-register-content"><form class="spell-filters" role="search"><label><span>Zauber finden</span><input type="search" name="search" placeholder="Name, Wirkung oder Voraussetzung …" autocomplete="off"></label><label><span>Grundgrad</span><select name="grade"><option value="all">Alle Grade</option>${Array.from({length:10},(_,grade)=>`<option value="${grade}">${grade ? `Grad ${grade}` : 'Zaubertrick'}</option>`).join('')}</select></label><label><span>Verwendung</span><select name="role"><option value="all">Alle Wirkungen</option>${['Schaden','Schutz','Formung & Nutzen','Ritual'].map(role=>`<option>${escape(role)}</option>`).join('')}</select></label></form><div class="spell-result-bar"><p data-role="result-count" role="status" aria-live="polite">72 Zauber im Verzeichnis</p><button type="button" data-action="reset-filters">Filter zurücksetzen</button><button type="button" data-action="collapse-spells">Alle schließen</button></div><p data-role="empty" hidden>Kein Zauber passt zu dieser Auswahl.</p>
...
</main><footer class="spell-page-footer"><a href="../index.html">← Die Kunst der Magie</a><span>Aleria · Elementarismus · Fassung 1</span><a href="#zauber">Zum Verzeichnis ↑</a></footer>
</body></html>\n`;
}
