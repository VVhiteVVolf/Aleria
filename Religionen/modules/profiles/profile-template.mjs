...
export function renderProfile(catalog, entry) {
  ...
  return renderShell({ outputPath, title: entry.title, description: entry.summary, main, sidebar, theme: collection?.theme, searchable: Boolean(entry.tradition), extraStyles: ['Religionen/modules/lore/lore.css', ...(entry.sections.some(section => section.disclosure) ? ['Religionen/modules/lore/lore-disclosure.css'] : []), ...(entry.tradition ? ['Religionen/modules/traditions/tradition.css'] : []), ...(entry.artifacts ? ['Religionen/modules/profiles/profile-artifacts.css'] : [])] });
}
