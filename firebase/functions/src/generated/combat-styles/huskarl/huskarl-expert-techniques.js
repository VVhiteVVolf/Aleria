...
import { getAdditionalHuskarlPathLessons } from './huskarl-additional-paths.js';

const levels = [9, 11, 13, 15, 17, 20];
const profiles = {
  skalde: {
    wall: [['Klinge des Hallenwächters','sidearm'],['Vers der festen Haltung','voice'],['Takt gegen den Waffenarm','sidearm'],['Schwert der treuen Wache','sidearm'],['Strophe vor dem Sturm','voice'],['Letzter Akkord der Wacht','sidearm']],
    advance: [['Hieb zum Aufbruch','sidearm'],['Schnitt gegen den Rhythmus','sidearm'],['Klinge der Heldensage','sidearm'],['Sax des Hallenrufers','sidearm'],['Betonter Schlussakkord','sidearm'],['Schwert der lebenden Sage','sidearm']]
  },
  skjoldr: {
...
export function getHuskarlExpertTechniques(classId) {
  ...
  const original = Object.entries(profile || {}).flatMap(([path, options]) => options.map(([name, weapon, mounted], index) =>
    createHuskarlTechnique(classId, F[path], { ...lessons[path][index], name, weapon, mounted,
      level: levels[index], slug: `${path}-${index + 1}` })));
  return [...original,...getAdditionalHuskarlPathLessons(classId).map(({path,...spec}) => createHuskarlTechnique(classId,F[path],spec))];
}
