...
import { getAdditionalHuskarlLessons } from './huskarl-additional-lessons.js';
...
export function getHuskarlBasicTechniques(classId) {
  const original = (lessons[classId] || []).map(([level, name, weapon, cost, effect, extra = {}], index) => {
  ...
  return [...original, ...getAdditionalHuskarlLessons(classId).map(spec => createHuskarlTechnique(classId,
    classId === 'hird-maid' && spec.level >= 6 ? F.militia : spec.level <= 6 ? F.foundation : F.advanced, spec))];
}
