...
export const HUSKARL_WEAPONS = Object.freeze({
  voice: { name: 'Stimme · nichtmagische Kampfansage', types: [] },
...
export function createHuskarlTechnique(classId, formId, spec) {
  ...
  if (spec.mobility) effects.push(temporaryCondition(id, 'Sicherer Schritt', `+${spec.mobility} m Bewegung für einen eigenen Beitrag; keine freie Bewegung oder automatische Loslösung.`,
    { movement: spec.mobility }, { target: 'self', on: spec.noDamage ? 'always' : 'hit', tags: 'Huskarl-Waffenlehre', slug: 'mobility' }));
  if (spec.temporaryHp) effects.push({ id: `${id}-resolve`, type: 'temporary-hit-points', target: 'self', on: 'always',
    formula: spec.temporaryHp, notes: 'Temporäre TP ersetzen nur einen niedrigeren vorhandenen Vorrat; sie werden nicht addiert.' });
  // Alternative lessons renew the same preparation instead of stacking it.
  // Effect IDs remain action-specific; the runtime uses condition IDs per caster.
  const preparationIds = { 'Feste Deckung':'huskarl-deckung', 'Gesammelte Ruhe':'huskarl-zielen', 'Sicherer Schritt':'huskarl-bewegung' };
  for (const effect of effects) {
    if (effect.condition && preparationIds[effect.condition.name]) effect.condition.id = preparationIds[effect.condition.name];
  }
  ...
  return { id, name: spec.name, minimumLevel: spec.level, status: 'draft', active: false, live: false,
    ...
    effects, secondarySave: spec.slow ? secondarySave(id, 'Gehemmter Schritt', '−2 m Bewegung für einen eigenen Beitrag nach misslungenem KRF-Rettungswurf; nicht additiv.', { movement: -2 }, { tags: 'Huskarl-Waffenlehre' })
      : spec.penalty ? secondarySave(id, 'Gebundene Waffenführung', '−1 Angriff für einen eigenen Beitrag nach misslungenem KRF-Rettungswurf; nicht additiv.', { attack: -1 }, { tags: 'Huskarl-Waffenlehre' }) : { enabled: false },
