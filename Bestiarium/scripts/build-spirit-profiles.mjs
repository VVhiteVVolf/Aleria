...
for (const id of SPIRIT_PROFILE_IDS) {
  ...
  if (profile.id !== id) throw new Error(`Spirit profile id does not match directory: ${id}`);
...
for (const [index, profile] of profiles.entries()) {
  ...
  const html = renderSpiritProfile(profile, navigation);
  const output = new URL('index.html', directory);

  if (check) {
    const existing = (await readFile(output, 'utf8')).replace(/\r\n/g, '\n');
    if (existing !== html) throw new Error(`Generated spirit profile is out of date: ${profile.id}. Run node Bestiarium/scripts/build-spirit-profiles.mjs`);
  } else {
    await writeFile(output, html, 'utf8');
  }
  console.log(`${check ? 'Checked' : 'Built'} spirit profile: ${profile.id}`);
}
