...
function siblingLabel(person, kind = 'full') {
  const prefix = kind === 'half' ? 'Halb' : '';
  return gendered(person, `${prefix}bruder`, `${prefix}schwester`, kind === 'half' ? 'Halbgeschwister' : 'Geschwister');
}
...
export function buildRelationshipMatrix(input, focusPersonId) {
  ...
  const parents = graph.getParents(focusPersonId);
  const siblings = graph.getSiblings(focusPersonId);
  const partners = graph.getPartners(focusPersonId);
  const children = graph.getChildren(focusPersonId);

  parents.forEach(parent => {
    add('ancestors', parent, parentLabel(parent), 'parent', 10);
    graph.getParents(parent.id).forEach(grandparent => {
      add('ancestors', grandparent, grandparentLabel(grandparent), 'grandparent', 20);
    });

    graph.getSiblings(parent.id).forEach(({ person: relative }) => {
      add('collateral', relative, gendered(relative, 'Onkel', 'Tante', 'Elterngeschwister'), 'aunt-uncle', 40);
      graph.getChildren(relative.id).forEach(cousin => {
  ...
  siblings.forEach(({ person: sibling, kind }) => {
    ...
    graph.getChildren(sibling.id).forEach(relative => {
  ...
  children.forEach(child => {
    ...
    graph.getChildren(child.id).forEach(grandchild => {
