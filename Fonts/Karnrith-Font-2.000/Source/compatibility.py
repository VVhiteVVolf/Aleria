"""Latin text extensions using the Karnrith Tiefenrunen master outlines.
...
def extend(glyphs,metrics,cmap,records):
    ...
    def put(name,g,adv,lsb=40):
        if name in supplied:return name
    ...
    def comp(name,parts,adv):
        return put(name,union_parts(glyphs,parts),adv)
    def ribbon(name,lines,adv,width=44):
        ...
        for points in lines:
            ...
            for i,(x,y) in enumerate(points):
                ...
                w=width*1.85*(.72+.28*math.sin(math.pi*t)**.5)/2
                left.append((x-dy/n*w,y+dx/n*w));right.append((x+dy/n*w,y-dx/n*w))
            # Union each ribbon segment so crossings stay filled rather than
            # cancelling through a self-intersecting polygon boundary.
            for i in range(len(left)-1):
                polygons.append(Polygon([left[i],left[i+1],right[i+1],right[i]]).buffer(0))
    ...
    # All basic Latin letters, sharp S, digits and six punctuation masters exist.
    svg('extensionDot','M0 30 L26 0 L0 -30 L-26 0 Z',0)
    charmap('ẞ',base('ß'))
    ...
    markdefs={
        ...
        0x303:('tilde','M-90 20 C-62 82 -28 82 5 47 C30 20 54 22 88 64 L88 28 C60 -17 27 -19 -7 18 C-34 46 -57 49 -90 -10 Z'),
        0x304:('macron','M-88 20 L88 35 L78 64 L-78 56 Z'),
        0x306:('breve','M-90 93 C-80 -13 80 -13 90 93 C46 22 -46 22 -90 93 Z'),
        0x307:('dot','M0 75 L26 40 L0 5 L-26 40 Z'),
        0x308:('dieresis','M-51 75 L-25 43 L-51 11 L-77 43 Z M51 75 L77 43 L51 11 L25 43 Z'),
    ...
    charmap('Ŋŋ','rune17');charmap('Þþ','rune21')
    ...
    svg('period','M115 123 L152 80 L115 37 L78 80 Z',230);charmap('.','period')
    ...
    svg('questionTop','M48 588 L48 664 L145 752 L333 752 L430 652 L430 532 L265 391 L265 260 L175 260 L175 432 L335 573 L335 621 L294 663 L178 663 L134 621 L134 588 Z',470)
    comp('question',[('questionTop',ident),('period',(1,0,0,1,104,0))],450);charmap('?','question')
    svg('exclamation','M70 755 L200 755 L180 725 L171 244 L99 244 L90 725 Z M135 132 L182 83 L135 34 L88 83 Z',280)
    ...
    svg('hyphen','M55 409 L385 409 L385 339 L55 339 Z',440);charmap('-‐‑','hyphen')
    ...
    svg('editorialRing','M380 760 C-87 760 -87 40 380 40 C847 40 847 760 380 760 Z M380 678 C725 678 725 122 380 122 C35 122 35 678 380 678 Z',760)
    comp('copyright',[('editorialRing',ident),(base('C'),(.43,0,0,.43,380-metrics[base('C')][0]*.215,224))],760);charmap('©','copyright')
    comp('registered',[('editorialRing',ident),(base('R'),(.43,0,0,.43,380-metrics[base('R')][0]*.215,224))],760);charmap('®','registered')
