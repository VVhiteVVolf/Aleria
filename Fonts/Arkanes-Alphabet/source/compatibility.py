...
def extend(glyphs,metrics,cmap,records):
    ...
    # K is an explicit input alias, preserving the existing /k/ sound of Coll.
    charmap('Kk',base('C'))
    # J, Q, W and sharp S reuse the calligraphic contour vocabulary.
    dia('extensionDot',0,0,20,25,0)
    jadv=metrics[base('I')][0]
    comp('latinJ',[(base('I'),(-1,0,0,1,jadv,0)),('extensionDot',(1,0,0,1,jadv-30,180))],jadv+45)
    charmap('Jj','latinJ')
    tail=svg('qTail','M235 265 C370 310 490 170 410 35 C375 -25 296 -4 300 48 C299 87 345 108 358 73 C337 52 335 35 361 29 C407 44 391 144 340 162 C309 178 267 175 235 265 Z',470)
    qadv=metrics[base('O')][0]
    comp('latinQ',[(base('O'),(.84,0,0,.84,30,105)),(tail,(.74,0,0,.74,135,15))],qadv)
    charmap('Qq','latinQ')
    vadv=metrics[base('V')][0]
    comp('latinW',[(base('V'),(.65,0,0,1,0,0)),(base('V'),(-.65,0,0,1,vadv*1.1,0))],vadv*1.1)
    charmap('Ww','latinW')
    sadv=metrics[base('S')][0]
    comp('sharpS',[(base('S'),(.75,0,0,.57,15,335)),(base('S'),(-.75,0,0,.57,sadv*.85,10))],sadv*.93)
    charmap('ßẞ','sharpS')
    ...
    markdefs={
        ...
        0x307:('dot','M0 75 L25 40 L0 10 L-25 40 Z'),
        0x308:('dieresis','M-51 78 L-25 43 L-51 8 L-77 43 Z M51 78 L77 43 L51 8 L25 43 Z'),
    ...
    charmap('ı',base('I'));charmap('ſ',base('S'));charmap('Ŋŋ','rune27');charmap('ĸ',base('K'))
    # Eng already has a canonical NG rune; thorn has canonical TH.
    charmap('Þþ','rune22')
    def pair(chars,a,b):
    ...
    dia('period',115,80,26,35,230);charmap('.','period')
    ...
    svg('exclamation','M110 735 L179 695 L145 245 L126 220 Z M135 130 L163 83 L135 42 L107 83 Z',280)
