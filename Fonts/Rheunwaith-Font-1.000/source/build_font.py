"""Rebuild Rheunwaith 2.000 from the included traced SVG master outlines.
...
def build():
    ...
    for i,record in enumerate(records):
        ...
        token='T' if record['token']=='Th' else record['token']
        if len(token)==1:
    ...
    for c,n in [('Q','extraQ'),('ß','extraSharpS')]:
    ...
    fb.setupNameTable({'familyName':'Rheunwaith','styleName':'Regular','uniqueFontIdentifier':'Rheunwaith-Gezeitenrunen-2.000','fullName':'Rheunwaith Regular','psName':'Rheunwaith-Regular','version':'Version 2.000','description':'Gezeitenrunen. 30 lore signs with CH LL NG RH TH ligatures, extended Latin and German text support.','licenseDescription':'SIL Open Font License 1.1. See OFL.txt. New artwork and glyph contours for the Aleria project.','licenseInfoURL':'https://openfontlicense.org/'})
    ...
    fb.font['head'].fontRevision=2.0
    ligatures='feature liga {\n'
    for token,target in [('CH','rune27'),('LL','rune25'),('NG','rune26'),('RH','rune28'),('TH','rune18')]:
    ...
    target=ROOT/'fonts/Rheunwaith-Regular.ttf';fb.save(target)
    ...
    print(f'Rheunwaith 2.000: {len(cmap)} codepoints, {len(glyphs)} glyphs; TTF, WOFF, WOFF2.')
