...
function formKind(form) {
  ...
  if (form.kind === 'duelist') return form.shortName === 'Freie Vertiefung' ? 'Vertiefungsphase' : 'Duellantenform';
  return 'Pfad';
}
...
export function renderCultureClassTraining(plan) {
  ...
  return `<section class="cenyr-training class-chapter" id="ausbildungsplan" aria-labelledby="heading-ausbildungsplan">
    ...
    ${renderFoundationSelection(plan)}
    <div class="cenyr-training-phases">${plan.trainingPhases.map(phase => `<article><span>Stufe ${phase.minimumLevel}–${phase.maximumLevel}</span><strong>${escape(phase.name)}</strong><small>${phase.kind === 'path-selection' ? 'Pfade wählen und Attackenbudget verteilen' : phase.kind === 'free-training' ? 'Pufferzone für eigene oder bestehende Techniken' : phase.kind === 'pending' ? 'Inhalt noch offen' : 'Ausbildungsabschnitt'}</small></article>`).join('')}</div>
